...
const ReusableQuill = forwardRef<ReusableQuillRef, ReusableQuillProps>(
  (
    ...
    if (!isClient) {
      return (
        <div className="h-32 bg-gray-50 rounded-md animate-pulse border" />
      );
    }

    // Calculate content height by subtracting toolbar height from total height
    const toolbarHeight = toolbar !== "none" ? "52px" : "0px"; // Approximate toolbar height
    const contentHeight = height 
      ? `calc(${typeof height === "string" ? height : `${height}px`} - ${toolbarHeight})`
      : undefined;

    return (
      <div
        className={`bg-white rounded-md border border-input overflow-hidden flex flex-col ${className}`}
        style={{
      ...
      >
        ...
        >
          ...
          >
            <EditorContent
              editor={editor}
              className="p-3 focus-within:outline-none h-full"
              style={{
                minHeight: typeof minHeight === "string" ? minHeight : `${minHeight}px`,
              }}
            />
